#!/bin/zsh
# ============================================================
# 打包 DEBUG 版本：APK (Android) + IPA (iOS 未签名) + DMG (macOS)
# 输出目录：build_output/
# ============================================================
set -e
...
log "清理旧构建（确保新图标/名称生效）"
flutter clean
log "获取依赖"
flutter pub get

# ---------- 1. Android APK (debug) ----------
log "[1/3] 构建 Android debug APK ..."
flutter build apk --debug --no-shrink
APK="build/app/outputs/flutter-apk/app-debug.apk"
if [ -f "$APK" ]; then
  cp "$APK" "$OUTPUT_DIR/lingua-link-debug.apk"
  ok "APK -> $OUTPUT_DIR/lingua-link-debug.apk ($(du -h "$OUTPUT_DIR/lingua-link-debug.apk" | cut -f1))"
else
  warn "APK 未生成"
fi

# ---------- 2. iOS IPA (debug, 未签名) ----------
log "[2/3] 构建 iOS debug 未签名 IPA ..."
cd ios
...
# ---------- 3. macOS DMG (debug) ----------
log "[3/3] 构建 macOS debug 应用并打包 DMG ..."
